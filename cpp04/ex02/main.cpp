...
/*   Updated: 2023/11/02 15:57:20 by tpetros          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void subjectTest(){
	...
	{
	std::cout << std::endl << "Half/Half AAnimal Array" << std::endl;
		AAnimal *animals[4];
		animals[0] = new Dog();
		animals[1] = new Cat();
		animals[2] = new Dog();
	...
	std::cout << std::endl << "SUBJECT TEST - pt3" << std::endl << std::endl;
	{
		...
		std::cout << "dogs[0] idea address: " << &(dogs[0]->getBrain()->getIdea(0)) << std::endl;
		std::cout << "dogs[1] idea address: " << &(dogs[1]->getBrain()->getIdea(1)) << std::endl;
		std::cout << "cats[0] idea address: " << &(cats[0]->getBrain()->getIdea(0)) << std::endl;
		std::cout << "cats[0] idea address: " << &(cats[1]->getBrain()->getIdea(1)) << std::endl;
		delete cats[1];
		delete dogs[1];
		delete dogs[0];
		delete cats[0];
...
// 	{
// 		AAnimal stack_animal;
// 		AAnimal *heap_animal = new AAnimal();
